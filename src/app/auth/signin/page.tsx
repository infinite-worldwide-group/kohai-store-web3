...
import { useMerchantLoginMutation } from "graphql/generated/graphql";
...
const SignIn: React.FC = () => {
  ...
  // GraphQL Mutation
  const [signIn] = useMerchantLoginMutation({
    onCompleted: (data) => {
      if (data && data.merchantLogin) {
        const { jwt, user, merchant, errors } = data.merchantLogin;

        if (!!jwt && user && merchant) {
          setJwtToken(jwt);
          setUser(user);
